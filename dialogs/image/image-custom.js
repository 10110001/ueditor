...
(function () {
    ...
    function initParam(){
    	editor.execCommand('serverparam', {'DocID': editor.getOpt('DocID'),"DocLibID":editor.getOpt("DocLibID")});
    }
    /* 初始化tab标签 */
    function initTabs() {
    ...
    function initButtons() {
        ...
        dialog.onok = function () {
            ...
            switch (id) {
                case 'remote':
                    list = remoteImage.getInsertList();//只返回各种属性列表
                    break;
                case 'upload':
    ...
    RemoteImage.prototype = {
        ...
        initEvents: function () {
            ...
            /* 改变url */
            domUtils.on($G("url"), 'keyup', addUrlChangeListener);//updatePreview       
            ...
            function addUrlChangeListener(){
				var value = $G("url").value;
					...
					var img = document.createElement("img"),						
						preview = $G("preview");
					/**
					//地址过滤??????问题：会把本地的图片，判断为不可用图片
					var imgTypeReg = /\.(png|gif|jpg|jpeg)$/gi, //格式过滤
						urlFilter = "";                         //地址过滤
					if (!imgTypeReg.test(imgurl) || imgurl.indexOf(urlFilter) == -1) {
            ...
            for (i = 0; i < this.imageList.length; i++) {
                ...
                list.push({
                   ...
                   // _src: prefix + data.url,
                	src: prefix + data.guid,
                     _src: prefix + data.guid,
                    title: data.title,
                    alt: data.original,
                    floatStyle: align
                });
